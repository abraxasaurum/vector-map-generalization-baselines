...
MODEL_FILE = PROJECT_ROOT / "outputs" / "transformer_baseline.pt"
HISTORY_FILE = PROJECT_ROOT / "outputs" / "transformer_training_history.csv"
FIGURE_FILE = PROJECT_ROOT / "outputs" / "transformer_test_predictions.png"
...
class ResidualPolygonTransformer(nn.Module):
    def __init__(self, n_points, d_model, n_heads, n_layers, feedforward_dim, dropout):
        super().__init__()

        self.input_projection = nn.Linear(2, d_model)
        self.position_embedding = nn.Parameter(
            torch.zeros(1, n_points, d_model)
        )
    ...
    def forward(self, x):
        embedding = self.input_projection(x) + self.position_embedding
        encoded = self.encoder(embedding)
        correction = self.output_projection(encoded)

        return x + correction


model = ResidualPolygonTransformer(
    n_points=n_points,
    d_model=D_MODEL,
    n_heads=N_HEADS,
    n_layers=N_LAYERS,
...
fig.suptitle(
    "Residual Transformer: source, Douglas-Peucker target and prediction",
    y=1.02,
)
plt.tight_layout()
